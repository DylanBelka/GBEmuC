...
void ld_r_r(u8 instruction, u8 src, bool is_lower_instr)
{
	...
	if (is_lower_instr)
	{
		u8 *dsts[] = { &registers.B, &registers.D, &registers.H, get_byte_ptr(registers.HL) };
		dst = dsts[dst_index];
	}
	else
	{
...
void bit_instruction_impl(u8 instr)
{
	...
	if (instr >= 0x40) // instructions from 0x40-0xFF have non register,
	{					// constant operands
...
void stop_cpu(void) { cpu.is_stopped = 1; }											// 0x10
...
void jr_n(void) { jr(TRUE); cpu.clock_cycles -= 4; }								// 0x18
...
void rra(void) { rl(&registers.A); }												// 0x1F
...
void reti(void) { ret(TRUE); cpu.IME = 1; cpu.clock_cycles -= 6; }					// 0xD9
...
void ld_a_cp(void) { registers.A = read_byte(registers.C + 0xFF00); }				// 0xF2
...
struct instruction instructions[256] =
{
	...
	{"call c, nn", call_c_nn, 0},
	{"0xDD undefined", undef_instr, 0},
	{"sbc a, n", sbc_a_n, 2},
	{"rst 0x18", rst_0x18, 0},
	{"ld (0xFF00 + n), a", ld_zpg_a, 2},
	{"pop hl", pop_hl, 1},
	{"ld (c), a", ld_pc_a, 1},
	{"0xE3 nop", nop, 1},
...
const int bit_instruction_ticks[256] =
{
